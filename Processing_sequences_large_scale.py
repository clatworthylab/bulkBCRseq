...
def get_info(file):
    (id, sample, info, gene, directory, pair, pair_final, dir, platform, spec,
     primer, other, reverse_primer_group) = ([], [], [], [], [], [], [], [],
                                             [], [], [], [], [])
    fh = open(file, "r")
    ind = 0
    for l in fh:
        ind = ind + 1
        if (l[0] != "#" and len(l) > 3):
            l = l.replace('/lustre/scratch108/viruses/rbr1/',
            ...
            if (len(l) >= 12):
                ...
                other.append('')
    fh.close()
    return (id, sample, info, gene, directory, pair, pair_final, dir, platform,
            spec, primer, other, reverse_primer_group)
...
if (len(args) < 5):
    ...
    try:
        (ids_, samples_, directories_,
         downsample_) = get_info_concat(concat_file)
    except:
    ...
    for i in range(0, len(samples_)):
        if "downsample_" in locals():
            id_, sample_, directory_, dwnsamp_ = ids_[i], samples_[
                i], directories_[i], downsample_[i]
        else:
    ...
    (ids, samples, infos, gene, source, pairs, pairs_final, dirs, platform,
     spec, primer, others, reverse_primer_group) = get_info(file)
    print(dir)
    dir_source_code = "/lustre/scratch117/cellgen/team297/kt16/BCRSeq/"
    ...
    for i in range(0, len(samples)):
        info, sample, gene_types, pair, pair_final, dir, platforms, primers, other = infos[
            i], samples[i], gene[i], pairs[i], pairs_final[i], dirs[
                i], platform[i], primer[i], others[i]
        bsub = ''
        if ("3" in command):
            span = '-n10 -R"span[hosts=1]"'
        else:
        ...
        if (bsub_command == "Y"):
            bsub = ("bsub -P team205 -G teichlab " + queue +
                    " -o logs/out_SPLITTING_" + id + " " + mem + " " + span +
                    " ")
        if ("1" in command):
            command1 = ("python " + dir_source_code +
                        "BIN/Read_processing_and_quality.py " + dir + " " +
                        id + " " + sample + " " + gene_types + " " + pair +
                        " " + species + " " + sources + " " + str(200) + " " +
                        primers + " " + platforms + " 1 " + other + " " +
        ...
        if ("3" in command):
            command1 = ("python " + dir_source_code +
                        ...
                        id + " " + sample + " " + gene_types + " " + pair +
                        " " + species + " " + sources + " " + str(200) + " " +
                        primers + " " + platforms + " 3 " + other + " " +
                        reverse_primer_group[i])
            commands.append(bsub + command1)
        if ("4" in command):
            command2 = ("python " + dir_source_code +
                        "BIN/Generate_repertoire_statistics.py " + dir +
                        "ORIENTATED_SEQUENCES/ANNOTATIONS/ " + id + " " + dir +
                        "ORIENTATED_SEQUENCES/NETWORKS/Fully_reduced_" + id +
                        ".fasta " + dir +
                        "ORIENTATED_SEQUENCES/Filtered_ORFs_sequences_all_" +
                        id + ".fasta " + gene_types + " " + species + " " +
                        dir +
                        "ORIENTATED_SEQUENCES/NETWORKS/Cluster_identities_" +
                        id + ".txt ANNOTATE,STATISTICS " +
                        reverse_primer_group[i])
            commands.append(bsub + command2)
        if ("4.5" in command):
            command3 = ("python " + dir_source_code +
                        "BIN/Local_immune_repertoire_annotator.py " + dir +
                        "ORIENTATED_SEQUENCES/ANNOTATIONS/TEST/ " + id + " " +
                        dir + "ORIENTATED_SEQUENCES/NETWORKS/Fully_reduced_" +
                        id + ".fasta " + gene_types + " " + species)
            commands.append(bsub + command3)
        if ("ISO1" in command):
            command1 = ("python " + dir_source_code + "BIN/IsoTyper_1.0.py " +
                        id + " " + id + " " + dir + " " + species + " " +
                        reverse_primer_group[i])
            commands.append(bsub + command1)
        if ("NONISO1" in command):
            command1 = ("python " + dir_source_code +
                        "BIN/Non_isotyper_1.0.py " + id + " " + id + " " +
                        dir + " " + species + " ")
            commands.append(bsub + command1)
    ...
    desc = file.replace("Samples_", "").replace(".txt", "")
    if ("10" in command):
        command10 = ("python " + dir_source_code +
                     "BIN/Get_figure_params.py " + dirss + " " + desc + " " +
                     idss)
        commands.append(bsub + command10)
    if ("10.5" in command):
        command10 = ("python " + dir_source_code +
                     "BIN/Get_coloured_figures.py " + dirss + " " + desc +
                     " " + idss)
        commands.append(bsub + command10)
    if ("11" in command):
        command11 = ("R CMD BATCH " + dirs[0] +
                     "ORIENTATED_SEQUENCES/Network_generation_" + desc + ".R")
