...
def main(argv):
    ...
    att_file = directory+'Att_'+sample+'_cloned.txt'
    out_file = directory+'Formatted_Att_'+sample+'_cloned.txt'
    cluster_file = directory+'Plot_ids_'+sample+'_cloned.txt'
    ...
    for c, v in cluster_value.items():
    	cluster_dict[c]= i
    	i += 1
    cluster[3] = [cluster_dict[c] for c in cluster[0]]
    cluster_id_dict = dict(zip(cluster[1], cluster[0]))
    cluster_sorted_dict = dict(zip(cluster[1], cluster[3]))
	
    att[3] = [str(cluster_id_dict[c]) if c in cluster_id_dict.keys() else None for c in att[0]]
    
    att[4] = [str(cluster_sorted_dict[c]) if c in cluster_sorted_dict.keys() else None for c in att[0]]
    
    top_10_clusters = [str(x) for x in list(cluster_value.index[:10])]
    top_10_clusters_dict = dict(zip(top_10_clusters, ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J']))
    att[5] = att[3]
    for x in att.index:
    ...
    isotype = {}
    for c in att[0]:
        isotype[c] = [int(x) for x in c.split('__')[1].split('|')[0].split('_')]
    isotype_df = pd.DataFrame.from_dict(isotype, orient = 'index')
    isotype_df.columns = ['IGHA', 'IGHD', 'IGHE', 'IGHG1', 'IGHG2A', 'IGHG2B', 'IGHG2C', 'IGHG3', 'IGHM']
    isotypes = list(isotype_df.idxmax(axis=1))
    ...
    if '-_-' in att[0][0]:
    	tissue = []
    	tissue_conversion = {'bm':'bone_marrow', 'l':'lung', 't':'tongue', 'b':'bladder', 'm':'meninges', 'ln':'mesenteric_lymph_nodes', 'd':'duodenum', 'i':'ilieum', 'cp':'colon_proximal', 'cd':'colon_distal'}
    ...
    node_file = directory+'Formatted_Att_'+sample+'_cloned.txt'
    edge_file = directory+'Edges_'+sample+'_cloned.txt'
    ...
    # remove_ids = [v.index for v in g.vs if v['size'] == 1]
    ...
    visual_style = {}
    visual_style['vertex_size'] = [3 + s/max_freq*30 for s in g.vs['size']]
    ...
    #isotype_col_dict = {'IGHA':'#4e79a7', 'IGHD':'#f28e2b', 'IGHE':'#e15759', 'IGHG1':'#76b7b2', 'IGHG2A':'#59a14f', 'IGHG2B':'#edc948', 'IGHG2C':'#b07aa1', 'IGHG3':'#ff9da7', 'IGHM':'#9c755f', 'NaN':'#f2f2f2', np.nan:'#f2f2f2'}
    #tissue_col_dict = {'bone_marrow':'#4E79A7', 'lung':'#A0CBE8', 'tongue':'#F28E2B', 'bladder':'#FFBE7D', 'meninges':'#59A14F', 'mesenteric_lymph_nodes':'#8CD17D', 'duodenum':'#B6992D', 'ilieum':'#F1CE63', 'colon_proximal':'#499894', 'colon_distal':'#86BCB6'}
    isotype_col_dict = {'IGHA':'#d62728', 'IGHD':'#ff7f0e', 'IGHE':'#2ca02c', 'IGHG1':'#1f77b4', 'IGHG2A':'#1f77b4', 'IGHG2B':'#1f77b4', 'IGHG2C':'#1f77b4', 'IGHG3':'#1f77b4', 'IGHM':'#9467bd', 'NaN':'#f2f2f2', np.nan:'#f2f2f2'}
    tissue_col_dict = {'bone_marrow':'#2ca02c', 'lung':'#ff7f0e', 'tongue':'#8c564b', 'bladder':'#9467bd', 'meninges':'#d62728', 'mesenteric_lymph_nodes':'#79706e', 'duodenum':'#1f77b4', 'ilieum':'#1f77b4', 'colon_proximal':'#1f77b4', 'colon_distal':'#1f77b4'}
    clone_col_dict = {'A':'#4e79a7','B':'#a0cbe8','C':'#f28e2b','D':'#ffbe7d','E':'#59a14f','F':'#8cd17d','G':'#b6992d','H':'#f1ce63','I':'#e15759','J':'#ff9d9a','K':'#f2f2f2'}

    # file names
    outI = directory+sample+'_cloned_isotype.png'
    outC = directory+sample+'_cloned_top_10_clones.png'
    outT = directory+sample+'_cloned_tissue.png'
    ...
    visual_style['vertex_frame_color'] = [clone_col_dict[i] for i in g.vs['clone']]
