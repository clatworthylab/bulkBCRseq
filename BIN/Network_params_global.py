...
def Get_differential_stats(
    cluster_file, id, dir, network_statistics, gene, species, gene_freq_file
):
    """Summary
    ...
    for l in fh:
        if l[0] != "#":
            ...
            (
                ...
                vrenyi,
                crenyi,
            ) = (l[2], l[4], l[5], l[6], l[7], l[8], l[9], l[10])
    fh.close()
    # largest v (%)
    fh = open(cluster_file, "r")
    ...
    edge_file = (
        dir.replace("ANNOTATIONS/", "NETWORKS/") + "Edges_" + id + ".txt"
    )
    fh = open(edge_file, "r")
    total_e, max_c_e = 0, 0
    for l in fh:
