...
def Deconvolute_edges(seq_file,  att_file, file_vertex, file_seqs, tmp_file0, file_edges,read_number_division, tmp_file1):
...
def Decon_identical2(seq_file, att_file, file_vertex, file_seqs, tmp_file,read_number_division):
    fh = open (seq_file , "r")
    all,seqs,freq_id ={},{},{}
    for header,sequence in fasta_iterator(fh):
        seqs[header.split("__")[0]]=sequence
...
def Deconvolute_edges2(seq_file,  att_file, file_vertex, file_seqs, tmp_file0, file_edges,read_number_division, tmp_file1):
    Decon_identical2(seq_file, att_file, file_vertex, file_seqs, tmp_file0,read_number_division)
    Decon_edges(att_file,file_seqs,file_edges,file_vertex,seq_file, tmp_file1)
    Reduce_edges (tmp_file1, file_edges)
    return()
...
def concat_fasta(concatenated_name, path, inputfiles):
    ...
    outfasta = path+'ORIENTATED_SEQUENCES/NETWORKS/Fully_reduced_'+str(concatenated_name)+'.fasta'
...
def Get_cluster_sizes_single2(file, downsample = None):
  ...
  df['sequence_id'] = [s+'-_-'+f for s, f in zip(df['sample_id'], df['sequence_id'])]
...
att_file = dir+"ORIENTATED_SEQUENCES/NETWORKS/Vertex_relations_"+id+"_cloned.txt"
...
cluster_file = dir+"ORIENTATED_SEQUENCES/NETWORKS/Cluster_identities_"+id+"_cloned.txt"
Reduced_file = dir+"ORIENTATED_SEQUENCES/NETWORKS/"+id+"_genotyped_germ-pass_clone-pass_fully_reduced.fasta"
airr_file = dir+"ORIENTATED_SEQUENCES/NETWORKS/"+id+"_genotyped_germ-pass_clone-pass.tsv"
checked_edges = dir+"ORIENTATED_SEQUENCES/NETWORKS/Checked_edges_"+id+"_cloned.txt"
plot_ids_file = dir+"ORIENTATED_SEQUENCES/NETWORKS/Plot_ids_"+id+"_cloned.txt"
file_seqs = dir+"ORIENTATED_SEQUENCES/NETWORKS/Sequences_"+id+"_cloned.txt"
tmp_file0 = dir+"ORIENTATED_SEQUENCES/NETWORKS/Decon_0_"+id+"_cloned.txt"
tmp_reduced_sequences = dir+"ORIENTATED_SEQUENCES/NETWORKS/Primary_reduced_sequences"+id+"_cloned.fasta"
...
Deconvolute_edges2(Reduced_file,  att_file, file_vertex, file_seqs, tmp_file0, file_edges,read_number_division, tmp_file1)
