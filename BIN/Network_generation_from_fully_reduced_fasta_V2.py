...
def Check_barcodes_MALBAC(
    ...
    outs = [
        "",
        "#ID\tnumber_of_reads\ttotal_reads_with_BC\tJ_barcode\tV_barcode\tbp_mismatches_to_consensus\tBC_accepted\tconsensus\tsequence\n",
    ]
...
def Single_J_barcoded_trimming(
    regions_J,
    J_primer,
    J1,
    J2,
    ...
    Print_trimmed_sequences(
        Output_trim, primer_tag_file, primer_tag_file_count, inside, ref_const
    )
    return ()
...
def Read_untrimmed_file(
    ...
    if inside == 1:
        ...
        if reverse_primer_group == "ISOTYPER" or reverse_primer_group == "ISO":
            ...
            fh = open(
                "/nfs/users/nfs_r/rbr1/IMMUNOLOGY/NETWORK_METHODS/STANDARD_BCR_PROCESSING/LIBRARY/Annealing_IsoTyper_constant_region_distinction_formatted_"
                + species
                + ".txt",
                "r",
            )
...
def Read_untrimmed_file_double1(
    ...
    for seq in seqs1:
        ...
        if passes == 1:
                ...
                if max(pv + [-1]) != -1:
                    ...
                    if pv - len(regions_V[V_primer[i]]) + len(V_primer[i]) > -1:
                        passes = Check_barcode(
                            seq[
                                pv
                                - len(regions_V[V_primer[i]])
                                + len(V_primer[i]) : pv
                                + len(V_primer[i])
                            ],
                            V_primer[i],
                            regions_V[V_primer[i]],
                        )
                        if passes == 1:
                            v_found = v_found + number
                            v_tag = seq[
            ...
            if passes == 0:
                for i in range(0, vs):
                    pre_bc_region = V_primerfull[i][
                        0 : V_primerfull[i].index("N")
                    ]
                    pre_bc_region2 = pre_bc_region[
                        len(pre_bc_region) / 2 : len(pre_bc_region)
                    ]
    ...
    status = (
        ...
        + "\tTotal sequences:\t"
        + str(total)
        + "\tNumber of V primers found:\t"
        + str(v_found)
        + "\tNumber of J primers found:\t"
        + str(J_found)
        + "\tPassed sequences:\t"
        + str(indp)
    )
...
def Split_reads_and_join_pre(
    ...
    print(
        id + "\tTotal sequences:",
        total,
        "\tNumber of joined sequences:",
        joining,
        "Number of barcodes matched:",
        primer_match,
        "% retained:",
        primer_match * 100.0 / total,
        "IDs saved",
        count_barcode_saved,
    )
    return ()
...
def Blast_match_J_const(out, seqs, Trim1, Trim2, refj, e_value, indent):
    ...
    for l in fh:
        l = l.strip().split()
        header, gene = l[0].split("|")[1].split("_"), map(
            int, l[0].split("|")[0].split("__")[1].split("_")
        )
...
def dfgdFG():
    ...
    print(
        "\nTotal sequences:",
        number_seqs,
        "\tNumber passed with open reading frame:",
        number_accepted,
        "\tPercentage accepted:",
        number_accepted * 100.0 / number_seqs,
        "%",
    )
    return ()
...
def Get_read_report(
    ...
    if method == "Multiplex_FORWARD_BARCODE_GROUPED":
        Seq_file1, Seq_file2 = (
            dir + "FASTQ_FILES/Sequences_" + barcode_group + "_1.fasta",
            dir + "FASTQ_FILES/Sequences_" + barcode_group + "_2.fasta",
        )
    rc = {}
...
def Get_read_report1(
    ...
    primer_matching, orf = Get_reduced_number_sequences(
        Trim1
    ), Get_reduced_number_sequences(nn_orf_filtered)
...
def Decon_edges(
    att_file, file_seqs, file_edges, file_vertex, seq_file, tmp_file1
):
    """Summary
    ...
    for c in same1:
        ...
        for i in sub_same:
            ...
            for j in sub_same[i]:
                ...
                freq = map(
                    int,
                    j1.split(read_number_division)[1].split("|")[0].split("_"),
                )
                if total == 0:
                    total = freq
                else:
...
def Get_network_clusters(file_vertex, file_edges, cluster_file):
    ...
    Output_cluster_file(G, cluster_file, file_vertex)
    return ()
...
def Output_cluster_file(G, cluster_file, file_vertex):
    """Summary
...
def Generate_networks(
    Sequence_file, tmp_file1, edge_lengths, tmp_file, file_out
):
    """Summary
...
def Intialise_files(dir):
    ...
    for d in dirs_to_add:
        ...
        if c.count("No such file or directory") == 1:
            commands.getoutput("mkdir " + dir + d)
    return ()
...
def QC_samples(dir, gene, id, source, length, species, barcode_group):
    ...
    if 1 == 1:
        ...
        if 1 == 1:
            ...
            command1 = (
                "java -jar ~sw10/QUASR_v7.01/qualityControl.jar -f "
                + reads1
                + " -o "
                + dir
            ...
            command3 = (
                ...
                + "FASTQ_FILES/Sequences_"
                + id
                + "_1.qc.fq | perl -e '$i=0;while(<>){if(/^\@/&&$i==0){s/^\@/\>/;print;}elsif($i==1){s/\./N/g;print;$i=-3}$i++;}' > "
                + dir
                + "FASTQ_FILES/Sequences_"
                + id
                + "_1.fasta"
            ...
            command4 = (
                ...
                + "FASTQ_FILES/Sequences_"
                + id
                + "_2.qc.fq | perl -e '$i=0;while(<>){if(/^\@/&&$i==0){s/^\@/\>/;print;}elsif($i==1){s/\./N/g;print;$i=-3}$i++;}' > "
                + dir
                + "FASTQ_FILES/Sequences_"
                + id
                + "_2.fasta"
...
def Decon_identical2(
    ...
    for i in sub_same:
        ...
        for j in sub_same[i]:
            j1 = freq_id[j.split("__")[0]]
            freq = map(
                int, j1.split(read_number_division)[1].split("|")[0].split("_")
            )
            if total == 0:
                total = freq
            else:
...
def concat_fasta(concatenated_name, path, inputfiles):
    """Summary

    Parameters
    ----------
    concatenated_name : TYPE
        Description
    path : TYPE
        Description
    inputfiles : TYPE
    ...
    for f in inputfiles:
        ...
        seqs = {}
        for header, sequence in fasta_iterator(fh):
            newheader = str(f) + "-_-" + str(header)
            seqs[newheader] = sequence
        fh.close()
        out = ""
        for l in seqs:
            out = ">" + l + "\n" + seqs[l] + "\n"
            Write_output(out, outfasta)


def Get_cluster_sizes_single2(file, downsample=None):
    """Summary
    ...
    cluster = Tree()
    for c in df.index:
        cluster[str(df.at[c, "clone_id"])][c].value = 1
    print(len(cluster))
    sizes = []
    for c in cluster:
        tot = len(cluster[c])
...
def Get_cluster_similarities_single2(seqs, cluster, file_out):
    """Summary
...
def Generate_networks2(Sequence_file, airr_file, file_out, downsample):
    """Summary
    ...
    TYPE
        Description
    """
    (s_sizes, cluster) = Get_cluster_sizes_single2(airr_file, downsample)
    (seqs) = Get_seqs_single(Sequence_file)
    Get_cluster_similarities_single2(seqs, cluster, file_out)
    return ()
...
att_file = (
    dir + "ORIENTATED_SEQUENCES/NETWORKS/Vertex_relations_" + id + "_cloned.txt"
)
file_vertex = dir + "ORIENTATED_SEQUENCES/NETWORKS/Att_" + id + "_cloned.txt"
file_edges = dir + "ORIENTATED_SEQUENCES/NETWORKS/Edges_" + id + "_cloned.txt"
cluster_file = (
    dir
...
Reduced_file = (
    ...
    + "ORIENTATED_SEQUENCES/NETWORKS/"
    + id
    + "_genotyped_germ-pass_clone-pass_fully_reduced.fasta"
)
airr_file = (
    dir
    + "ORIENTATED_SEQUENCES/NETWORKS/"
    + id
    + "_genotyped_germ-pass_clone-pass.tsv"
)
checked_edges = (
    dir + "ORIENTATED_SEQUENCES/NETWORKS/Checked_edges_" + id + "_cloned.txt"
)
plot_ids_file = (
    dir + "ORIENTATED_SEQUENCES/NETWORKS/Plot_ids_" + id + "_cloned.txt"
)
file_seqs = (
    dir + "ORIENTATED_SEQUENCES/NETWORKS/Sequences_" + id + "_cloned.txt"
)
tmp_file0 = dir + "ORIENTATED_SEQUENCES/NETWORKS/Decon_0_" + id + "_cloned.txt"
...
Generate_networks2(Reduced_file, airr_file, att_file, downsample)
Deconvolute_edges2(
    Reduced_file,
    att_file,
    file_vertex,
