...
def Get_library(ref, end, max_len):
    ...
    for header, sequence in fasta_iterator(fh):
        ...
        if (end == "end"):
            sequence = sequence[len(sequence) - max_len - ident:len(sequence) -
                                ident]
        else:
            if (end == "start"):
...
def Blast_match_D(out, refd, tmp_file, seqs, passed, file_D_genes,
                  batch_number):
    fh = open(tmp_file, "w")
    fh.write(out)
    fh.close()
    # command1 = ("/software/pubseq/bin/ncbi_blast/blastall -p blastn -a 1 -d " +
    #             refd + " -e 2 -i " + tmp_file + " -o " + tmp_file +
    #             "_dblast -b 1 -m 8 -W 4")
    command1 = (bin_path + "blastall -p blastn -a 1 -d " + refd + " -e 2 -i " +
                tmp_file + " -o " + tmp_file + "_dblast -b 1 -m 8 -W 4")
    os.system(command1)
    ...
    for id in genes:
        ...
        out = out + id + "\t" + a[0] + "\t" + str(start) + "\t" + str(
            end) + "\t" + seqs[id][0][a[2]:a[3]] + "\n"
        passed = passed + 1
...
def Locate_CDR3_start_site_IGH(CDR3_prot, seq, shift, codon, cdr3_ends,
                               cdr3_ends_near):
        ...
        if (passed != 2):
            for trial in cdr3_ends_near:
                ...
                if (re.search(r"" + trial, CDR3_prot)):
                    ind = CDR3_prot.index(
                        re.search(r"" + trial,
                                  CDR3_prot).group(0)) - cdr3_ends_near[trial]
                    CDR3_prot_trim = CDR3_prot[0:ind]
                    if (len(CDR3_prot_trim) > 0):
                        if ((type(len(options)) is int) & (type(CDR3_prot_trim) is str)):
...
def Blast_match(out, refv, refj, tmp_file, out1, indent, seqs, lengthv, prots,
                codon, gene, cdr3_ends, cdr3_ends_near):
    fh = open(tmp_file, "w")
    fh.write(out)
    fh.close()
    # blast = "/software/pubseq/bin/ncbi_blast/blastall"
    blast = bin_path + "blastall"
    command1 = (blast + " -p blastn -a 10 -d " + refv + " -e 1e-5 -i " +
                tmp_file + " -o " + tmp_file + "_vblast -b 1 -m 8 -W 10")
    command2 = (blast + " -p blastn -a 10 -d " + refj + " -e 10 -i " +
                tmp_file + " -o " + tmp_file + "_jblast -b 1 -m 8 -W 4")
    os.system(command1)
    fh = open(tmp_file, "w")
    fh.write(out1)
    fh.close()
    ...
    for l in fh:
        ...
        if (int(l[6]) < int(l[7]) and int(l[8]) < int(l[9])):
            if (l[0] in seqs):
                ...
                passesj[l[0]] = (l[1].upper(), int(l[6]) + addition,
                                 int(l[7]) + addition, int(l[8]), int(l[9]),
    ...
    for l in fh:
        ...
        if (len(l) >= 10):
            if (int(l[6]) < int(l[7]) and int(l[8]) < int(l[9])):
                ...
                mut = int(l[4])
                if (l[0] in passesv):
                    additional_mutations, v_end_query = passesv[
                        l[0]][11], passesv[l[0]][13]
                    if (passesv[l[0]][13] < int(l[7])):
                        v_end_query, v_end_ref = int(
                            l[7]), int(l[7]) + len(lengthv[l[1]]) - int(l[9])
                        nn_start = passesv[l[0]][15]
                        CDR3_start = passesv[l[0]][9]
                        additional_mutations1 = max(
                            [nn_start - v_end_query, 0])
                        if (additional_mutations1 < passesv[l[0]][11]):
                            additional_mutations = additional_mutations1
                    mut, ins = int(l[4]) + passesv[l[0]][6], int(
                        l[5]) + passesv[l[0]][7]
                    passesv[l[0]] = (passesv[l[0]][0], passesv[l[0]][1],
                                     passesv[l[0]][2], passesv[l[0]][3],
                                     passesv[l[0]][4], passesv[l[0]][5], mut,
                                     ins, passesv[l[0]][8], passesv[l[0]][9],
                                     passesv[l[0]][10], additional_mutations,
                                     passesv[l[0]][12], v_end_query,
                                     passesv[l[0]][14], v_end_query)
                else:
                    mut = int(l[4])
                    v_end_query, v_end_ref = int(
                        l[7]), int(l[7]) + len(lengthv[l[1]]) - int(l[9])
                    ...
                    if (l[0].split("__")[0] in prots):
                        ...
                        if (gene == "IGH"):
                            CDR3_p, passed, nn_start, CDR3_nn = Locate_CDR3_start_site_IGH(
                        ...
                        if (gene.count("TCR") == 1):
                            ...
                            #     CDR3_prot_potential, seqs[l[0]], shift, codon)
                            CDR3_start, CDR3, additional_mutations, CDR3_found, CDR3_nn, nn_start = (
                                v_end_query, "-", v_end_query, "NO", "-",
                                v_end_query)
                            passed = 0
                        if (passed != 0):
                            CDR3_start, CDR3, additional_mutations, CDR3_found = nn_start, CDR3_p, max(
                                [nn_start - v_end_query, 0]), "YES"
                            if (additional_mutations > 0):
                                # print "R:",lengthv[l[1]][int(l[8]):int(l[9])]
                                # print "Q:",seqs[l[0]][int(l[6]):int(l[7])]
                                ref_gene = lengthv[
                                    l[1]][int(l[9]):len(lengthv[l[1]])]
                                query_seq = seqs[l[0]][int(l[7]):CDR3_start]
                                minl = min([len(ref_gene), len(query_seq)])
                                ref_gene, query_seq = ref_gene[
                                    0:minl], query_seq[0:minl]
                                additional_mutations = len([
                                    i for i in range(len(ref_gene))
                            ...
                            CDR3_start, CDR3, additional_mutations, CDR3_found, CDR3_nn, nn_start = (
                                v_end_query, "-", v_end_query, "NO", "-",
                                v_end_query)
                        passesv[l[0]] = (v_gene, int(l[6]), int(l[7]),
                                         int(l[8]), int(l[9]), int(l[3]), mut,
                                         int(l[5]), end_diff, CDR3_start, CDR3,
                                         additional_mutations, CDR3_found,
                                         v_end_query, CDR3_nn, nn_start)
                    else:
                        print(l)
        else:
            print(l)
...
def Get_annotation(passesv, passesj, seqs, out_file, lengthj, regions, failed,
                   CDR3_end, prots, codon, gene):
    # head = (
    ...
    for id in seqs:
        ...
        if (id in passesv):
            v_found = [
                passesv[id][0], passesv[id][1], passesv[id][2], passesv[id][3],
                passesv[id][4], passesv[id][5], passesv[id][6], passesv[id][7]
            ]
            o = o + "\t" + v_found[0]
            ...
            if (id in passesj):
                j_found = [
                    passesj[id][0], passesj[id][1], passesj[id][2],
                    passesj[id][3], passesj[id][4], passesj[id][5],
                    passesj[id][6], passesj[id][7]
                ]
...
def Get_region_boundaries(species, loc):
    ...
    for l in fh:
        ...
        if (id.count("VH") == 1):
            id.replace("VH", "IGHV")
        id = id.split("*")[0].upper()
        regions[id] = (int(l[1]), int(l[2]), int(l[3]), int(l[4]), int(l[5]),
                       int(l[6]), int(l[7]), int(l[8]), int(l[9]), int(l[10]))
    fh.close()
    fh = open(J_ref, "r")
    CDR3_end = {}
    for l in fh:
        if (l[0] != "#"):
            l = l.strip().split()
            id, CDR3_end_nn, CDR3_end_aa = l[0].split("*")[0], int(l[3]), int(
                l[2])
...
def Identify_D_genes(annot_file, file_D_genes, refd, tmp_file, seq_file):
    ...
    for header, seq in fasta_iterator(fh):
        if (header in D_approx_region):
            ...
            if (len(seq_d_poss) > 5):
                ...
                if (ind > batch):
                    passed = Blast_match_D(out, refd, tmp_file, seqs, passed,
                                           file_D_genes, batch_number)
    ...
    fh.close()
    print("\nTotal:", total, "Passed:", passed, "% Passed:",
          passed * 100.0 / total)
...
def Assign_sequences(Seq_file, gene, annot_file, tmp_file, species,
                     protein_file, refv, refj, loc, CDR3_end_file, codon_file):
    # ids_test = Get_ids_test(
    ...
    for header, seq in fasta_iterator(fh):
        ...
        if (1 == 1):
            ...
            if (ind >= batch):
                batch_number = batch_number + 1
                (passesv, passesj) = Blast_match(out, refv, refj, tmp_file,
                                                 out1, indent, seqs, lengthv,
                                                 prots, codon, gene, cdr3_ends,
                                                 cdr3_ends_near)
                (failed) = Get_annotation(passesv, passesj, seqs, annot_file,
                                          lengthj, regions, failed, CDR3_end,
                                          prots, codon, gene)
                ...
                #       end=' ')
                print(
                    "\rBatch number:" + batch_number +
                    "\tSequences completed:" + t + "\t% accepted",
                    (t - failed) * 100.0 / t + "%")
    if (len(seqs) > 0):
        (passesv, passesj) = Blast_match(out, refv, refj, tmp_file, out1,
                                         indent, seqs, lengthv, prots, codon,
                                         gene, cdr3_ends, cdr3_ends_near)
        (failed) = Get_annotation(passesv, passesj, seqs, annot_file, lengthj,
                                  regions, failed, CDR3_end, prots, codon,
                                  gene)
    t2 = time.time()
    fh.close()
    print(
        "\n", Seq_file + "\t" + gene + "\tV\t" + str(t) + "\t" +
        str(round((t2 - t1), 3)) + "\t sec\t" +
        str(round((t2 - t1) * 100000.0 / (60 * t), 3)) + "\tmins/100,000\t" +
        str(failed) + "\tFAILED")
    os.system("rm " + tmp_file + " " + tmp_file + "_jblast " + tmp_file +
              "_vblast")
    return ()
    ...
    for id in seqs:
        if (id != max_id):
            if (clusters[id.split("__")[0]] == max_c):
                ...
                seq2, seqs2aa, v, j = seqs[id][1], seqs[id][2], seqs[id][
                    3], seqs[id][4]
                l2 = len(seq2)
                if (len(max_aa)
                        in [len(seqs2aa) - 1,
                            len(seqs2aa),
                            len(seqs2aa) + 1] and v == max_v and j == max_j):
                    if (max_seq == seq2):
                        max_cluster[id] = max_c
                    else:
                        (s1, s2, p1,
                         trimmed) = Trim_sequences(max_seq, seq2, l1, l2,
                                                   mismatch)
                ...
                if (l[15].count("CDR3_UNCERTAIN") != 1):
                    if (l[15] != "CDR3_UNDETERMINED"):
                        ...
                        if (len(CDR3) < 25 or (CDR3[0] == "C" and CDR3[len(CDR3) - 1] == "W")):
                            ...
                            seqs[l[0]] = [
                                freq, l[18], l[17], l[1][0:5], l[13][0:4]
                            ]
                            if (max_f < freq):
    ...
    if (len(seqs) > 0):
        ...
        for id in annot:
            ...
            if (id in max_cluster):
                cluster = "MAX_CLUSTER"
            out = out + annot[id] + "\t" + str(max_cluster[id]) + "\t" + str(
                len(seqs[id][2])) + "\t" + str(
                    id.split("__")[1]) + "\t" + cluster + "\n"
            ind = ind + 1
            if (ind > 500):
...
def Get_network_statistics_per_chain(cluster_file, sample, dir,
    ...
    for l in fh:
        if (l[0] != "#"):
            l = l.strip().split()
            id = l[2]
            chains, freq, id_short = id.split("|")[1].split("_"), list(
                map(int,
                    id.split("__")[1].split("|")[0].split("_"))), id.split(
                        "__")[0]
            t1 = t1 + sum(freq)
            n = n + 1
            ...
            if (len(total_v) == 0):
                total_v, total_reads = [0] * len(chains_short), [
                    0
                ] * len(chains_short)
    ...
    for c1 in chains_short:
        ...
        if (len(cluster_sizes_sub) > 0):
            ...
            vgini, cgini = Gini_index(cpoints, cvdf, vpoints, vvdf,
                                      sum(cluster_sizes_sub), sum(sizesv[c1]),
                                      total_v)
            max_pop, max_1_pop = cpoints[len(cpoints) - 1] * 100.0 / sum(
                sizesv[c1]), cpoints[len(cpoints) - 2] * 100.0 / sum(
                    sizesv[c1])
            out = out + str(sample) + "\t" + c1 + "\t" + str(sum(
                sizesv[c1])) + "\t" + str(len(sizesv[c1])) + "\t" + str(
                    vgini) + "\t" + str(cgini) + "\t" + str(
                        max_pop) + "\t" + str(max_1_pop) + "\n"
...
def Get_cluster_vertex_distributions(cluster_file):
    ...
    return (cpoints, cvdf, vpoints, vvdf, totalc, totalv, totalreads, c_sizes,
...
def Proportional_measures(c_sizes, totalreads):
    ...
    (max_pop, max_1_pop) = (s[0][1] * 100.0 / totalreads,
...
def Get_network_statistics(cluster_file, id, dir, network_statistics, gene,
                           species, cluster_size_distribution,
                           vertex_size_distribution):
    (cpoints, cvdf, vpoints, vvdf, totalc, totalv, totalreads, c_sizes,
     vertices_in_max_cluster) = Get_cluster_vertex_distributions(cluster_file)
    Print_distributions(vpoints, vvdf, vertex_size_distribution)
    Print_distributions(cpoints, cvdf, cluster_size_distribution)
    (vrenyi, crenyi) = Renyi_entropy(cpoints, cvdf, vpoints, vvdf, totalc,
                                     totalv, totalreads)
    (vgini, cgini) = Gini_index(cpoints, cvdf, vpoints, vvdf, totalc, totalv,
                                totalreads)
...
def Get_large_clusters(cluster_file, threshold,
                       max_number_of_clusters_to_include):
    fh = open(cluster_file, "r")
    ...
    for c in clust_size:
        ...
        if (prop >= threshold):
            perc_network, number_vertices, number_reads, max_seq, reads_in_max_seq = prop, len(
                clusters[c]), 0, '', 0
            ...
            sig_clust_info.append([
                c, perc_network, number_vertices, number_reads, max_seq,
                reads_in_max_seq
            ])
                ...
                if (len(l) >= 14):
                    ...
                    if (len(l) >= 20 and l[19].count("CDR") == 0):
                        ...
                        if (c in cluster_mutations):
                            cluster_mutations[c] = copy.deepcopy(
...
def Get_cluster_stats(annot_file, cluster_file, id, dir, network_statistics,
                      gene, species, seq_file, loc):
    ...
    if (len(sig_clust_info) > 0):
        for i in range(0, len(sig_clust_info)):
            c, perc_network, number_vertices, number_reads, max_seq, reads_in_max_seq = sig_clust_info[
                i]
...
def Get_CDR3_lengths_of_interest(annot_file, CDR3_length_file, sample, dir):
    ...
    if (sample in ["01", "03", "05", "09"]):
        llama = "37"
    # prot_cluster_file = dir.replace(
    #     "ANNOTATIONS/",
    #     "TEMPORAL_PROTEIN/") + "Protein_Cluster_IDs_" + llama + ".txt"
    seq_proteins_all = dir.replace(
        "ANNOTATIONS/",
        "TEMPORAL_PROTEIN/") + "Protein_Fully_reduced_" + llama + ".fasta"
    ...
    for l in fh:
        if (l[0] != "#"):
            ...
            if (len(l) >= 13):
                if (l[0].count("__") != 0):
                    f, v, j = sum(
                        map(int, l[0].split("__")[1].split("|")[0].split(
                            "_"))), l[1].split("*")[0], l[13].split("*")[0]
                else:
...
protein_file = sys.argv[
    4]  # protein fasta (does not matter if read number index is not correct)
gene = sys.argv[5]  # either IGH, IGL, IGK, TRA, TRB, TRD, TRG
species = sys.argv[6]  # HOMO_SAPIENS...
cluster_file = sys.argv[
    7]  # cluster file (output from Read_processing_and_quality.py program)
command = sys.argv[
    8]  # Comma separated list: ANNOTATE = Generate raw V-J annotation files, STATISTICS = Get population statistics
forward_primer_group = sys.argv[
    9]  # Forward primers used (basically to disinguish between ISO_DD and IsoTyper)
...
CDR3_cluster_file = cluster_file.replace("Cluster_identities_",
                                         "Cluster_CDR3_defined_identities_")
custom_reference_blast_file = dir + "Custom_reference_blast_file_" + id + ".txt"
pre_seq_file = dir.replace("ANNOTATIONS/",
                           "") + "TMP/Trimmed_orientated_all_" + id + ".fasta"
...
print("constant_region:", constant_region, "isotyper_primer_set:",
      isotyper_primer_set)
...
if ("ANNOTATE" in command):
    if (species in [
            "HOMO_SAPIENS", "MACACA_MULATTA", "MUS_MUSCULUS", "LLAMA_GLAMA"
    ]):
        Assign_sequences(seq_file, gene, annot_file, tmp_file, species,
                         protein_file, refv, refj, loc, CDR3_end_file,
                         codon_file)
...
if ("ANNOTATE_B" in command):
    ...
    if (gene not in ["TCR"]):
        Get_CDR3_lengths(annot_file, CDR3_length_file, id, dir)
...
if ("STATISTICS" in command):
    ...
    Get_network_statistics(cluster_file, id, dir, network_statistics, gene,
                           species, cluster_size_distribution,
                           vertex_size_distribution)
    Get_cluster_stats(annot_file, cluster_file, id, dir, cluster_statistics,
                      gene, species, seq_file, loc)
    if (constant_region == "TRUE"):
        Get_network_statistics_per_chain(cluster_file, id, dir,
                                         per_chain_repertoire_statistics_file,
                                         isotyper_primer_set)
